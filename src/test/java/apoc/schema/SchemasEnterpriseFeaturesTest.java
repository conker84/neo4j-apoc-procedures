...
public class SchemasEnterpriseFeaturesTest {
    ...
    public void testSchemaRelationshipsExclude() {
        ...
        testResult(session, "CALL apoc.schema.relationships({excludeRelationships:['LIKED']})", (result) -> {
    ...
    public void testSchemaNodesExclude() {
        session.writeTransaction(tx -> {
            tx.run("CREATE CONSTRAINT ON (book:Book) ASSERT book.isbn IS UNIQUE");
            tx.success();
            return null;
        });
        testResult(session, "CALL apoc.schema.nodes({excludeLabels:['Book']})", (result) -> {
            assertTrue(!result.hasNext());
        });
        session.writeTransaction(tx -> {
            tx.run("DROP CONSTRAINT ON (book:Book) ASSERT book.isbn IS UNIQUE");
            tx.success();
            return null;
        });
    }
