...
class AWSProcedures {
    ...
    fun sentiment(@Name("apiKey") apiKey: String,
            ...
            : Stream<MapResult> = AWSClient(apiKey, apiSecret, log!!).sentiment(data, config).stream()
    ...
    fun keyPhrases(@Name("apiKey") apiKey: String,
                   ...
                   @Name("data") data: Any,
                   @Name(value = "config", defaultValue = "{}") config: Map<String, Any>)
            : Stream<MapResult> = AWSClient(apiKey, apiSecret, log!!).keyPhrases(data, config).stream()

    @Procedure(value = "apoc.ai.aws.vision", mode = Mode.READ)
    @Description("Provides a entity analysis for provided text")
    fun vision(@Name("apiKey") apiKey: String,
               @Name("apiSecret") apiSecret: String,
               @Name("data") data: Any,
               @Name(value = "config", defaultValue = "{}") config: Map<String, Any>): Stream<MapResult> = AWSClient(apiKey, apiSecret, log!!).vision(data, config).stream()
